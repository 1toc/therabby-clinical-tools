(() => {
  ...
  class WebHidWbbDevice {
    ...
    async disconnect(){
      ...
      if(this.device){this.device.removeEventListener('inputreport',this.boundInput);if(this.device.opened)await this.device.close();}
  ...
  class LoadBalanceViewerApp {
    constructor(){
      ...
      this.log=[];this.logging=false;this.sessionId=null;
      this.recentBase=[];
    ...
    startLog(){this.log=[];this.logging=true;this.sessionId=this.makeSessionId();$('logButton').textContent='■ LOG STOP';$('logButton').classList.add('logging');$('logState').textContent=this.sessionId;this.showAlert('センサーログの記録を開始しました。','ok');}
    stopLog(){this.logging=false;$('logButton').textContent='● LOG START';$('logButton').classList.remove('logging');$('logState').textContent=`${this.log.length} samples`;if(!this.log.length)return this.showAlert('記録されたデータがありません。');this.downloadLogs();this.showAlert('CSVとsession.jsonを保存しました。','ok');}
    logSample(frame,m){const relX=m.weightPresent&&this.center?m.copX-this.center.x:null,relY=m.weightPresent&&this.center?m.copY-this.center.y:null;this.log.push({timestamp_ms:Math.round(performance.now()),session_id:this.sessionId,raw_lf:frame.raw?.lf??'',raw_rf:frame.raw?.rf??'',raw_lb:frame.raw?.lb??'',raw_rb:frame.raw?.rb??'',lf_kg:m.lf,rf_kg:m.rf,lb_kg:m.lb,rb_kg:m.rb,total_kg:m.total,left_pct:m.leftPct,right_pct:m.rightPct,front_pct:m.frontPct,back_pct:m.backPct,cop_x_norm:m.copX,cop_y_norm:m.copY,relative_cop_x:relX,relative_cop_y:relY,weight_present:m.weightPresent,connection_state:this.mode==='mock'?'mock':'connected'});}
    downloadLogs(){
      const headers=['timestamp_ms','session_id','raw_lf','raw_rf','raw_lb','raw_rb','lf_kg','rf_kg','lb_kg','rb_kg','total_kg','left_pct','right_pct','front_pct','back_pct','cop_x_norm','cop_y_norm','relative_cop_x','relative_cop_y','weight_present','connection_state'];
      const cell=(v)=>v===null||v===undefined?'':typeof v==='number'?v.toFixed(6):String(v);
      const csv=[headers.join(','),...this.log.map(r=>headers.map(h=>cell(r[h])).join(','))].join('\n');
      this.downloadBlob(`${this.sessionId}_sensor_log.csv`,'text/csv;charset=utf-8',csv);
      const session={session_id:this.sessionId,application:'Therabby Load Balance Viewer',version:'0.1',mode:this.mode,zero_applied:Object.values(this.zero).some(v=>Math.abs(v)>1e-6),center_applied:!!this.center,weight_threshold_kg:this.threshold,samples:this.log.length,created_at:new Date().toISOString()};
      setTimeout(()=>this.downloadBlob(`${this.sessionId}_session.json`,'application/json;charset=utf-8',JSON.stringify(session,null,2)),200);
    }
    downloadBlob(name,type,content){const blob=new Blob([content],{type});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
    makeSessionId(){const d=new Date(),p=n=>String(n).padStart(2,'0');return `${d.getFullYear()}${p(d.getMonth()+1)}${p(d.getDate())}_${p(d.getHours())}${p(d.getMinutes())}${p(d.getSeconds())}`;}
